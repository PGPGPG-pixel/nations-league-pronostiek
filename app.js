    ...
    if (user) {
      ...
      db.collection('groups').where('memberIds', 'array-contains', currentUser.id)
            .onSnapshot(snap => {
              ...
              // merge remote groups with any locally created groups so we don't delete user-created groups
              let localGroups = [];
  ...
  try {
    db.collection('matches').onSnapshot(snap => {
      ...
      snap.forEach(doc => {
        const d = doc.data() || {};
        if (!d.id) d.id = doc.id;
        // convert Firestore Timestamp to simple 'YYYY-MM-DD HH:MM' string if needed
...
async function processLiveFeedData(data) {
  ...
  for (const m of data) {
    ...
    // determine incoming scores
    const h = (typeof m.homeScore !== 'undefined') ? Number(m.homeScore) : (typeof m.home_homeScore !== 'undefined' ? Number(m.home_homeScore) : null);
    const a = (typeof m.awayScore !== 'undefined') ? Number(m.awayScore) : (typeof m.away_awayScore !== 'undefined' ? Number(m.away_awayScore) : null);
...
function renderAdmin(container) {
  const view = document.createElement('div');
  view.className = 'admin-view';
  view.innerHTML = `<h2>Admin</h2><div class="admin-section">Simpele match beheer (demo)</div>`;
  container.appendChild(view);
}
